...
post '/generate' do
  ...
  total = arr.inject{ |sum, el| sum + el }.to_f

  arr.each_with_index do |pred, index|
    hash_arr << {:party => @@mapping[index], :percentage => 100*pred/total, :colour => colour_for(@@mapping[index])} unless index == 5
  end

  puts hash_arr.map{|h| h[:percentage] }.inject{ |sum, el| sum + el }.to_f

  max_hash = hash_arr.sort_by{|hash| hash[:percentage] }.last
  max_party = "We predict that the party best suiting you is #{max_hash[:party]}"

  content_type :json

  {:parties => hash_arr, :mp => lowest_mp[1], :colour => colour_for(lowest_mp[0]), :max_party => max_party}.to_json
end
        ...
        $('form').submit(function() {
          ...
          $.ajax({
            ...
            success: function(raw_data) {
              ...
              $('div#worded_result').html(raw_data['max_party']);
              $('div#mp_result').css('color', raw_data['colour']);
            }
          });
...
<div id="worded_result">
</div>
In terms of individual MPs, your views are the closest to: 
<div id="mp_result">
</div>
